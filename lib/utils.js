...
exports.getExifColorProfile = function getExifColorProfile(image, cb) {
    async.parallel({
        hasEmbeddedICCProfile: function (cb) {
            colors.hasEmbeddedICCProfile(image, function (err, data) {
                cb (err, data);
            });
        },
        getInteropIndex: function (cb) {
            colors.getInteropIndex(image, function (err, data) {
                cb(err, data);
            });
        }
    }, function (err, data) {
        if (data.hasEmbeddedICCProfile) {
            cb(null, 'embedded');
        } else if (data.getInteropIndex === 'R98') {
            cb(null, 'sRGB');
        } else if (data.getInteropIndex === 'R03') {
